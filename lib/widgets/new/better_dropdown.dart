...
const double _kMenuItemHeight = 36;
...
class _DropdownMenuPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    _painter.paint(
      canvas,
      rect.topLeft,
      new ImageConfiguration(size: rect.size),
    );
  }
...
class _DropdownMenu<T> extends StatefulWidget {
  const _DropdownMenu({Key? key, this.padding, this.route, this.color})
    : super(key: key);
...
class _DropdownMenuState<T> extends State<_DropdownMenu<T>> {
  ...
  Widget build(BuildContext context) {
    ...
    for (int itemIndex = 0; itemIndex < route.items!.length; ++itemIndex) {
      ...
      children.add(
        Material(color: widget.color, child: route.items![itemIndex]),
      );
    }
    ...
    this.openChild,
       ...
       assert(
         value == null ||
             items
                     .where(
                       (BetterDropdownMenuItem<T> item) => item.value == value,
                     )
                     .length ==
                 1,
       ),
  ...
  Widget build(BuildContext context) {
    ...
    return new Semantics(
      ...
      child: Material(
        ...
        child: new InkWell(onTap: _handleTap, child: result),
      ),
    );
  }
}
...
class BetterDropdownMenuItem<T> extends StatelessWidget {
  ...
  const BetterDropdownMenuItem({Key? key, required this.child, this.value})
    : super(key: key);
