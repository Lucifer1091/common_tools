...
class _InfinityCradleState extends State<InfinityCradle>
  ...
  void initState() {
    ...
    animation = _rotationTween.animate(animationController)..addListener(() {
      setState(() {});
    });
          ...
          child: Row(
            ...
            children: [
              Transform.rotate(
                ...
                child: Dot(size: widget.size / 3, color: widget.color),
              ),
              Transform.translate(
                offset: Offset(animationController.value * -widget.size / 3, 0),
                child: Row(
                  children: [
                    Dot(size: widget.size / 3, color: widget.color),
                    Dot(size: widget.size / 3, color: widget.color),
                  ],
                ),
              ),
...
class Dot extends StatelessWidget {
  const Dot({required this.color, required this.size, super.key});
