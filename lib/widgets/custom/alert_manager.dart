...
typedef AlertBuilder =
    Widget Function(AlertData data, {Color? backgroundColor, TextStyle? style});

/// The data that an Alert needs.
typedef AlertData =
    ({String id, AlertType type, String message, Duration duration});

/// Default alert builder to easily set it up
Widget defaultAlertBuilder(
  AlertData data, {
  Color? backgroundColor,
  TextStyle? style,
}) => _Alert(data: data, backgroundColor: backgroundColor, style: style);

class _Alert extends StatelessWidget {
  const _Alert({required this.data, this.backgroundColor, this.style});
        ...
        child: Container(
          ...
          child: Row(
            children: [
              ...
              const Space.w16(),
              Expanded(child: Text(data.message, style: style)),
            ],
          ),
        ),
      ),
