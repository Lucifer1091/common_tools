...
class JsonViewState extends State<JsonView> {
  ...
  List<Widget> _getList() {
    ...
    for (final entry in widget.jsonObj.entries) {
      ...
      if (ink) {
        list.add(getContentWidget(entry.value as Object));
      }
    }
    return list;
  }

  static StatefulWidget getContentWidget(Object content) {
    if (content is List) {
      return JsonArrayViewerWidget(content, notRoot: true);
    } else {
      return JsonView(content as Map<dynamic, dynamic>, notRoot: true);
    }
  }
  ...
  List<Widget> _getList() {
    ...
    for (final dynamic content in widget.jsonArray) {
      ...
      list
        ...
        ..add(const SizedBox(height: 4));
      if (ink) {
        list.add(JsonViewState.getContentWidget(content as Object));
      }
      i++;
    }
    return list;
