                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      if (widget.model.useYear)
                        useAll()
                            ? SizedBox(child: buildList(context, 0), width: 64)
                            : Expanded(child: buildList(context, 0))
                      else
                        Container(),
                      if (widget.model.useMonth)
                        Expanded(child: buildList(context, 1))
                      else
                        Container(),
                      if (widget.model.useDay)
                      ...
                      if (widget.model.useWeekDay)
                        ...
                        Container(),
                      if (widget.model.useHour)
                        Expanded(child: buildList(context, 4))
                      else
                        Container(),
                      if (widget.model.useMinute)
                        Expanded(child: buildList(context, 5))
                      else
                        Container(),
                      if (widget.model.useSecond)
...
class DatePickerModel {
  ...
  DatePickerModel({
    ...
    assert(
      !useWeekDay || (!useSecond && !useMinute && !useHour),
  ...
  void setInitialTime() {
    dateStart = List.generate(
      6,
      (index) => index < dateStart.length ? dateStart[index] : 0,
    );
    ...
    dateEnd = List.generate(
      6,
      (index) => index < dateEnd.length ? dateEnd[index] : 0,
    );
    ...
    if (dateInitial != null) {
      var initList = List.generate(
        6,
        (index) => index < dateInitial!.length ? dateInitial![index] : 0,
      );
  ...
  void setInitialYearData() {
    var years = List.generate(
      dateEnd[0] - dateStart[0] + 1,
      (index) => index + dateStart[0],
    );
    data[0] =
        useYear && filterItems != null
  ...
  void setInitialMonthData() {
    ...
    if (dateEnd[0] == dateStart[0]) {
      month = List.generate(
        dateEnd[1] - dateStart[1] + 1,
        (index) => index + dateStart[1],
      );
  ...
  void setInitialTimeData() {
    ...
    if (dateStart.length > 3) {
      if (!useYear &&
        ...
        hour = List.generate(
          max(0, dateEnd[3] - dateStart[3] + 1),
          (i) => i + dateStart[3],
        );
        minute = List.generate(
          max(0, dateEnd[4] - dateStart[4] + 1),
          (i) => i + dateStart[4],
        );
        second = List.generate(
          max(0, dateEnd[5] - dateStart[5] + 1),
          (i) => i + dateStart[5],
        );

        data[4] =
            useHour && filterItems != null
                ? filterItems!(DateTypeKey.hour, hour)
                : hour;
        data[5] =
            useMinute && filterItems != null
                ? filterItems!(DateTypeKey.minute, minute)
                : minute;
        data[6] =
            useSecond && filterItems != null
      ...
      if (initialTime.hour >= dateStart[3]) {
        hour = List.generate(
          24 - dateStart[3],
          (index) => index + dateStart[3],
        );
      }
      if (initialTime.minute >= dateStart[4]) {
        minute = List.generate(
          60 - dateStart[4],
          (index) => index + dateStart[4],
        );
      }
      if (initialTime.second >= dateStart[5]) {
        second = List.generate(
          60 - dateStart[5],
          (index) => index + dateStart[5],
        );
      }
    }
    data[4] =
