...
class CancelableRetry<T> {
  ...
  CancelableOperation<dynamic>? _delay;
  bool _running = false;
  ...
  Duration _getDelay(int attempt) {
    assert(attempt > 0, 'Attempts must be greater than zero');

    final rndFactor = 1 + randomizationFactor * (_rnd.nextDouble() * 2 - 1);

    // prevent overflows
    final exp = min(attempt, 31);
    final delay = delayFactor * pow(2.0, exp) * rndFactor;
...
class CancelableOperation<T> {
  ...
  static CancelableOperation<void> fromSubscription(
    ...
    final completer = CancelableCompleter<void>(onCancel: subscription.cancel);
    subscription
  ...
  static CancelableOperation<T> race<T>(
    ...
    final completer = CancelableCompleter<T>(onCancel: cancelAll);
    for (final operation in operations) {
      operation.then(
        (value) {
          if (!done) {
            unawaited(
              cancelAll().whenComplete(() => completer.complete(value)),
            );
          }
        },
  ...
  Future<T?> valueOrCancellation([T? cancellationValue]) {
    ...
    unawaited(value.then(completer.complete, onError: completer.completeError));
  ...
  CancelableOperation<R> thenOperation<R>(
    ...
    if (_completer.isCanceled) {
      unawaited(cancelForwarder._forward());
    } else {
...
class CancelableCompleter<T> {
  ...
  ///
  /// The [_onCancel] function is called if the [operation] is canceled,
  /// by calling [CancelableOperation.cancel]
  /// before the operation has completed.
  /// If [_onCancel] returns a [Future],
  /// that future is also returned by [CancelableOperation.cancel].
  ///
  /// The [_onCancel] function will be called at most once.
  ...
  /// chained operations.
  List<_CancelForwarder<dynamic>>? _cancelForwarders;
  ...
  void completeOperation(
    ...
    _mayComplete = false;
    if (isCanceled) {
      if (propagateCancel) unawaited(result.cancel());
      result.value.ignore();
      return;
    }
    result.then<void>(
    ...
    if (propagateCancel) {
      unawaited(_cancelCompleter?.future.whenComplete(result.cancel));
    }
  }
  ...
  Future<void> _cancel() {
    ...
    if (cancelCompleter == null) return Future.value();
