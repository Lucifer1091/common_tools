...
class _DatePickerDialogState extends State<_DatePickerDialog> {
  ...
  void initState() {
    super.initState();
    _view = widget.initialValue?._view ?? _CalendarView.now();
  ...
  void _promoteView() {
    ...
    setState(() {
      _viewType = _viewType == MyCalendarViewType.date
...
abstract class CalendarValue {
  ...
  _CalendarValueLookup _lookup(int year, [int? month = 1, int? day = 1]);
...
class SingleCalendarValue extends CalendarValue {
  ...
  @override
  _CalendarValueLookup _lookup(int year, [int? month, int? day]) {
  ...
  @override
  _CalendarView get _view => date.toCalendarView();
...
class _CalendarState extends State<_Calendar> {
  ...
  void _handleTap(DateTime date) {
    ...
    if (widget.selectionMode == MyCalendarSelectionMode.multi) {
      ...
      final lookup = calendarValue._lookup(date.year, date.month, date.day);
  ...
  Widget build(BuildContext context) {
    return _CalendarGrid(
      ...
      itemBuilder: (item) {
        ...
        final lookup =
            widget.value?._lookup(date.year, date.month, date.day) ??
...
class _MonthCalendar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 1; i <= 12; i++) {
      ...
      _CalendarItemType type = _CalendarItemType.none;
      if (calendarValue != null) {
        final lookup = calendarValue!._lookup(date.year, date.month);
        switch (lookup) {
          case _CalendarValueLookup.none:
            if (now != null &&
...
class _YearCalendar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = yearSelectStart; i < yearSelectStart + 16; i++) {
      ...
      if (calendarValue != null) {
        final lookup = calendarValue!._lookup(date.year);
        switch (lookup) {
          case _CalendarValueLookup.none:
