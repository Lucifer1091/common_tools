...
typedef PageTransformerBuilderCallback =
    Widget Function(Widget child, TransformInfo info);

class PageTransformerBuilder extends PageTransformer {
  PageTransformerBuilder({required this.builder, super.reverse});
...
class _TransformerPageViewState extends State<TransformerPageView> {
  ...
  Widget _buildItem(BuildContext context, int index) {
    return AnimatedBuilder(
      ...
      builder: (BuildContext c, Widget? w) {
        final int renderIndex = _pageController.getRenderIndexFromRealIndex(
          index,
        );
        Widget child;
        child = widget.itemBuilder(context, renderIndex);

        if (_size == null) return child;

        double position = 0;

        final double page = _pageController.realPage ?? 0;
  ...
  Widget build(BuildContext context) {
    final IndexedWidgetBuilder builder = _transformer == null
        ? _buildItemNormal
        : _buildItem;
  ...
  void initState() {
    ...
    _pageController =
        ...
        TransformerPageController(
          ...
          loop: widget.loop,
          reverse: widget.transformer != null && widget.transformer!.reverse,
        );
