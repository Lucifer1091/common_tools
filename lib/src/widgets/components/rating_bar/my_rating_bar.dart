...
class _MyRatingBarState extends State<MyRatingBar>
      ...
      if (box != null) {
        ...
        if (distance < closestDistance) {
          ...
          if (widget.allowHalfRating) {
            ...
            closestRating = positionRelativeToStar < halfPoint
                ? i + 0.5
                : i + 1.0;
          } else {
  ...
  void _updateRating(double newRating) {
    ...
    if (_rating != newRating) {
      setState(() {
        _rating = newRating;
        _animation = Tween<double>(begin: _animation.value, end: _rating)
            .animate(
  ...
  Widget _buildRatingItem(int position) {
    final double fillLevel = (_animation.value - (position - 1)).clamp(
      0.0,
      1.0,
    );
...
class _RatingClipper extends CustomClipper<Rect> {
  ...
  bool shouldReclip(_RatingClipper oldClipper) {
    return fillLevel != oldClipper.fillLevel ||
