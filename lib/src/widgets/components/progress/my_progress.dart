...
import './my_progress_circular.dart';
...
class _ProgressIndicatorState extends State<ProgressIndicator>
  ...
  Widget _getDefaultLabelFromStatus(MyProgressStatus status) {
    ...
    Widget getAutoText() => showAutoText && widget.type != MyProgressType.micro
        ? Text('${(widget.value! * 100).round()}%')
        : const Text('');

    final statusWidgets = {
      MyProgressStatus.primary: getAutoText(),
      MyProgressStatus.warning: showInsideLabel
          ? getAutoText()
          : showIconBorder
          ? const Icon(Icons.error_rounded)
          : const Icon(CupertinoIcons.exclamationmark),
      MyProgressStatus.danger: showInsideLabel
          ? getAutoText()
          : showIconBorder
          ? const Icon(CupertinoIcons.xmark_circle_fill)
          : const Icon(Icons.close_rounded),
      MyProgressStatus.success: showInsideLabel
          ? getAutoText()
          : showIconBorder
      ...
      child: widget.showLabel
          ? Align(
              ...
              child: Padding(
                ...
                child: _buildLabelWidget(context.colorScheme.primaryForeground),
              ),
            )
          : null,
    );
  ...
  Widget _buildLabelWidget(Color labelColor) {
    ...
    switch (widget.type) {
      case MyProgressType.linear:
        ...
        fontWeight = _animation.value <= 0.1
            ? FontWeight.bold
            : FontWeight.normal;
      case MyProgressType.circular:
