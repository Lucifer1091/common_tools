...
class _MyStepsItems extends StatelessWidget {
  ...
  TextStyle titleStyle(BuildContext context, Color color) =>
      switch (size) {
        ...
        fontWeight: type == MyStepType.steps || type == MyStepType.timeline
            ? FontWeight.w600
  ...
  _MyStepStyle getStepStyle(BuildContext context) {
    ...
    // Final decoration
    if (type != MyStepType.icon) {
      iconDecoration = shouldSetIconWidgetDecoration
          ? iconDecoration ??
  ...
  Widget build(BuildContext context) {
    ...
    final iconContainerSize = type == MyStepType.timeline
        ? timelineSize
        : type == MyStepType.simple
        ? simpleIconSize
        : stepSize;

    final simpleSize = type == MyStepType.line
        ? lineHeight * 2
        : type == MyStepType.simple
        ? iconSize
        : iconContainerSize;

    final iconMarginBottom = type == MyStepType.simple
        ? iconMargin / 2
        : iconMargin;
    ...
    if (direction == Axis.vertical) {
      stepContent = Container(
        margin: type != MyStepType.timeline
        ...
        children: [
          ...
          if (data.customTitle != null)
            ...
            Container(
              ...
              alignment: type == MyStepType.line
