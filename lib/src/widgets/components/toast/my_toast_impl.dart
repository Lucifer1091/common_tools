...
class _MyToastImpl {
  ...
  static OverlayState? _resolveOverlayState({
    ...
    final OverlayState? navigatorOverlay = Navigator.maybeOf(
      safeContext,
      rootNavigator: true,
    )?.overlay;
  ...
  static _ToastStackTransform _resolveStackTransform({
    ...
    final alignedToasts = _activeToasts
        .where(
          (toast) =>
  ...
  static double _resolveExpandedOffset(
    ...
    for (int index = toastIndex + 1; index < alignedToasts.length; index++) {
      ...
      final double toastHeight = toast.height > 0
          ? toast.height
          : _estimatedToastHeight;
  ...
  static ToastController _showToast({
    ...
    final CapturedThemes? capturedThemes = (context?.mounted ?? false)
