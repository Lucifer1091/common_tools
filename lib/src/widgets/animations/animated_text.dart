...
class AnimatedTextReveal extends AnimatedText {
  ...
  Widget animatedBuilder(BuildContext context, Widget? child) {
    ...
    final scrambleFrame = (clampedProgress * _sourceCharacters.length * 12)
        .floor();
...
class AnimatedTextKit extends StatefulWidget {
  ...
  }) : assert(texts.length > 0, 'animatedTexts must not be empty'),
       assert(
         !isRepeatingAnimation || totalRepeatCount > 0 || repeatForever,
         'totalRepeatCount must be greater than 0 if isRepeatingAnimation is true and repeatForever is false',
       ),
       assert(
         null == onFinished || !repeatForever,
         'onFinished must be null if repeatForever is true',
       );
...
class _AnimatedTextKitState extends State<AnimatedTextKit>
  ...
  bool get _isLast => _index == widget.texts.length - 1;
  ...
  void _initAnimation() {
    _currentAnimatedText = widget.texts[_index];
  ...
  void _animationEndCallback(AnimationStatus state) {
    if (state == AnimationStatus.completed) {
      _setPause();
      assert(
        null == _timer || !_timer!.isActive,
        'Timer should not be active when animation completes',
      );
  ...
  void _onTap() {
    if (widget.displayFullTextOnTap) {
      if (_isCurrentlyPausing) {
        ...
        assert(
          null == _timer || !_timer!.isActive,
          'Timer should not be active when onTap is called',
        );
        _timer = Timer(
