...
abstract class MouseAreaRegistry {
  /// Register the given [MyMouseAreaRenderBox] with the registry.
  void registerMouseArea(MyMouseAreaRenderBox region);

  /// Unregister the given [MyMouseAreaRenderBox] with the registry.
...
class MouseAreaSurfaceRenderBox extends RenderProxyBoxWithHitTestBehavior
  ...
  final Set<MyMouseAreaRenderBox> _registeredRegions = <MyMouseAreaRenderBox>{};
  final Map<Object?, Set<MyMouseAreaRenderBox>> _groupIdToRegions =
      <Object?, Set<MyMouseAreaRenderBox>>{};
  ...
  void handleEvent(PointerEvent event, HitTestEntry entry) {
    ...
    final hitRegions = _getRegionsHit(
      _registeredRegions,
      result.path,
    ).cast<MyMouseAreaRenderBox>().toSet();

    final insideRegions = <MyMouseAreaRenderBox>{
      for (final MyMouseAreaRenderBox region in hitRegions)
  ...
  void registerMouseArea(MyMouseAreaRenderBox region) {
    ...
    if (region.groupId != null) {
      _groupIdToRegions[region.groupId] ??= <MyMouseAreaRenderBox>{};
  ...
  Set<HitTestTarget> _getRegionsHit(
    Set<MyMouseAreaRenderBox> detectors,
...
class MyMouseArea extends SingleChildRenderObjectWidget {
  /// Creates a const [MyMouseArea].
  ///
  ...
  /// Whether or not this [MyMouseArea] is enabled as part of the composite
  /// region.
  final bool enabled;

  /// How to behave during hit testing when deciding how the hit test propagates
  /// to children and whether to consider targets behind this [MyMouseArea].
  ///
  ...
  /// {@template MyMouseArea.groupId}
  /// An optional group ID that groups [MyMouseArea]s together so that they
  ...
  void updateRenderObject(
    ...
    covariant MyMouseAreaRenderBox renderObject,
  ) {
    renderObject
      ..registry = MouseAreaRegistry.maybeOf(context)
      ..enabled = enabled
...
class MyMouseAreaRenderBox extends RenderProxyBoxWithHitTestBehavior {
  /// Creates a [MyMouseAreaRenderBox].
  ...
  /// An optional group ID that groups [MyMouseAreaRenderBox]s together so
  ...
  /// The registry that this [MyMouseAreaRenderBox] should register with.
  ///
...
class MyMouseAreaSurface extends SingleChildRenderObjectWidget {
