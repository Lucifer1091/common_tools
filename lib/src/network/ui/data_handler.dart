...
class DataHandlerConfig {
  /// Creates the shared data handler configuration instance.
  ...
  static void setGlobalWidgets({
    ...
    if (loadingWidget != null) {
      setGlobalLoadingWidget(loadingWidget);
    }
    if (errorWidget != null) {
      setGlobalErrorWidget(errorWidget);
    }
    if (emptyWidget != null) {
      setGlobalEmptyWidget(emptyWidget);
    }
  }
}
...
class DataHandler<T> extends ChangeNotifier {
  ...
  void onSuccess(T newData) {
    final bool hasChanged = _state != DataState.success || _data != newData;
    if (hasChanged) {
  ...
  Widget when({
    ...
    return AnimatedBuilder(
      animation: this,
      builder: (BuildContext context, Widget? _) => _buildStateWidget(
        onSuccess: onSuccess,
        onLoading: onLoading,
        onError: onError,
        onEmpty: onEmpty,
  ...
  Widget whenSliverList({
    ...
    if (!enabled && _data != null) {
      final int count = itemCount(_data as T);
      return SliverList(
        delegate: SliverChildBuilderDelegate(
          (BuildContext context, int index) => itemBuilder(_data as T, index),
          childCount: count,
        ),
      );
    }
  ...
  Future<void> refresh(Future<T> Function() dataFetcher) async {
    startLoading();
    try {
      final T newData = await dataFetcher();
      onSuccess(newData);
    } catch (error) {
...
/// }
/// ```
extension DataHandlerList<T extends List<R>, R> on DataHandler<T> {
